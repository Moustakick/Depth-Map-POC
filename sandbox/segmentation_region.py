...
def neighbourPixelRegion(pos, height, width, pixelinregion, list_index_region): # retourne la liste des pixels voisins d'un pixel qui n'appartiennent pas à sa région
    listNeighbour = []
    i = pos//width
    j = pos%width
...
def list_adjacence(height, width, pixelinregion, tabRegion, list_region, list_index_region):
    list_adj = []
    for i in range(len(list_region)):
